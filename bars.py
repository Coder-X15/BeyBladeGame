...
class Bar(object):
    ...
    def on_update(self, new_value):
        ...
        relative_width = calc_relative_width(max_value=self._max_value, value=self._value, width=width)
        self._bar_rect = pygame.Rect(left, top, relative_width, height)
        return
...
class TextBar(Bar):
    ...
    def get_height(self):
        txt_height = self._text_obj.get_rect().height
        bar_height = self._bar_rect.height
        return max(txt_height, bar_height)
